...
import {
	...
	getRegisteredUserType,
	getRegisteredUserConnectionType,
...
const setupContainer = (decodedSessionToken) => {
	...
	container.register({
		...
		getUserFields: asFunction(getUserFields).scoped(),
		getUserType: asFunction(getUserType).scoped(),

		getActionPointPriorityFields: asFunction(getActionPointPriorityFields).scoped(),
		getActionPointPriorityType: asFunction(getActionPointPriorityType).scoped(),
		getActionPointPriorityConnectionType: asFunction(getActionPointPriorityConnectionType).scoped(),
		actionPointPriorityTypeResolver: asClass(ActionPointPriorityTypeResolver).scoped(),

		getActionPointReferenceFields: asFunction(getActionPointReferenceFields).scoped(),
		getActionPointReferenceType: asFunction(getActionPointReferenceType).scoped(),
		getActionPointReferenceConnectionType: asFunction(getActionPointReferenceConnectionType).scoped(),
		actionPointReferenceTypeResolver: asClass(ActionPointReferenceTypeResolver).scoped(),

		getActionPointStatusFields: asFunction(getActionPointStatusFields).scoped(),
		getActionPointStatusType: asFunction(getActionPointStatusType).scoped(),
		getActionPointStatusConnectionType: asFunction(getActionPointStatusConnectionType).scoped(),
		actionPointStatusTypeResolver: asClass(ActionPointStatusTypeResolver).scoped(),

		getActionPointFields: asFunction(getActionPointFields).scoped(),
		getActionPointConnectionType: asFunction(getActionPointConnectionType).scoped(),
		getActionPointType: asFunction(getActionPointType).scoped(),
		actionPointTypeResolver: asClass(ActionPointTypeResolver).scoped(),

		getDepartmentFields: asFunction(getDepartmentFields).scoped(),
		getDepartmentType: asFunction(getDepartmentType).scoped(),
		getDepartmentConnectionType: asFunction(getDepartmentConnectionType).scoped(),
		departmentTypeResolver: asClass(DepartmentTypeResolver).scoped(),

		getEmployeeFields: asFunction(getEmployeeFields).scoped(),
		getReportingEmployeeType: asFunction(getReportingEmployeeType).scoped(),
		getEmployeeType: asFunction(getEmployeeType).scoped(),
		getEmployeeConnectionType: asFunction(getEmployeeConnectionType).scoped(),
		employeeTypeResolver: asClass(EmployeeTypeResolver).scoped(),

		getManufacturerFields: asFunction(getManufacturerFields).scoped(),
		getManufacturerType: asFunction(getManufacturerType).scoped(),
		getManufacturerConnectionType: asFunction(getManufacturerConnectionType).scoped(),
		manufacturerTypeResolver: asClass(ManufacturerTypeResolver).scoped(),

		getMeetingDayFields: asFunction(getMeetingDayFields).scoped(),
		getMeetingDayType: asFunction(getMeetingDayType).scoped(),
		getMeetingDayConnectionType: asFunction(getMeetingDayConnectionType).scoped(),
		meetingDayTypeResolver: asClass(MeetingDayTypeResolver).scoped(),

		getMeetingDurationFields: asFunction(getMeetingDurationFields).scoped(),
		getMeetingDurationType: asFunction(getMeetingDurationType).scoped(),
		getMeetingDurationConnectionType: asFunction(getMeetingDurationConnectionType).scoped(),
		meetingDurationTypeResolver: asClass(MeetingDurationTypeResolver).scoped(),

		getMeetingFrequencyFields: asFunction(getMeetingFrequencyFields).scoped(),
		getMeetingFrequencyType: asFunction(getMeetingFrequencyType).scoped(),
		getMeetingFrequencyConnectionType: asFunction(getMeetingFrequencyConnectionType).scoped(),
		meetingFrequencyTypeResolver: asClass(MeetingFrequencyTypeResolver).scoped(),

		getMSOPFields: asFunction(getMSOPFields).scoped(),
		getMSOPType: asFunction(getMSOPType).scoped(),
		getMSOPConnectionType: asFunction(getMSOPConnectionType).scoped(),
		msopTypeResolver: asClass(MSOPTypeResolver).scoped(),

		getRegisteredUserFields: asFunction(getRegisteredUserFields).scoped(),
		getRegisteredUserType: asFunction(getRegisteredUserType).scoped(),
		getRegisteredUserConnectionType: asFunction(getRegisteredUserConnectionType).scoped(),
