...
const setupContainer = (decodedSessionToken) => {
	...
	container.register({
		...
		actionPointPriority: asClass(ActionPointPriority).scoped(),
		actionPointReference: asClass(ActionPointReference).scoped(),
		actionPointStatus: asClass(ActionPointStatus).scoped(),
		actionPoint: asClass(ActionPoint).scoped(),
		department: asClass(Department).scoped(),

		employee: asClass(Employee).scoped(),
		manufacturer: asClass(Manufacturer).scoped(),
		meetingDay: asClass(MeetingDay).scoped(),
		meetingDuration: asClass(MeetingDuration).scoped(),
		meetingFrequency: asClass(MeetingFrequency).scoped(),
		msop: asClass(MSOP).scoped(),
		registeredUser: asClass(RegisteredUser).scoped(),
