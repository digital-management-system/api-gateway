import { GraphQLObjectType, GraphQLID, GraphQLString, GraphQLNonNull, GraphQLInt } from 'graphql';
import { connectionDefinitions } from 'graphql-relay';

import { NodeInterface } from '../interface';
import Manufacturer from './Manufacturer';

export default class ActionPointPriority {
	static singleType = null;
	static connectionDefinitionType = null;

	constructor({ manufacturerDataLoader }) {
		ActionPointPriority.singleType = new GraphQLObjectType({
			name: 'ActionPointPriority',
			fields: () => ({
				id: { type: new GraphQLNonNull(GraphQLID), resolve: (_) => _.get('id') },
				name: { type: new GraphQLNonNull(GraphQLString), resolve: (_) => _.get('name') },
				manufacturer: {
					type: new GraphQLNonNull(Manufacturer.singleType),
					resolve: async (_) => manufacturerDataLoader.getManufacturerLoaderById().load(_.get('manufacturerId')),
				},
			}),
			interfaces: [NodeInterface],
		});

		ActionPointPriority.connectionDefinitionType = connectionDefinitions({
			name: 'ActionPointPriorities',
			nodeType: ActionPointPriority.singleType,
			connectionFields: {
				totalCount: {
					type: GraphQLInt,
					description: 'Total number of action point priorities',
				},
			},
		});
	}

	getType = () => ActionPointPriority.singleType;

	getConnectionDefinitionType = () => ActionPointPriority.connectionDefinitionType;
}
